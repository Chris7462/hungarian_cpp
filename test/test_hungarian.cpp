...
TEST_F(HungarianTest, LargeMatrix20x8)
{
  ...
  for (int i = 0; i < assignment.size(); ++i) {
    if (assignment(i) >= 0) {
      EXPECT_FALSE(usedColumns[assignment(i)])
        << "Column " << assignment(i) << " assigned multiple times";
...
TEST_F(HungarianTest, PerformanceTest)
{
  ...
  std::cout << "Performance test (50x50): " << cost
...
TEST_F(HungarianTest, ProfitMaximizationMatrix6x6)
{
  ...
  for (int i = 0; i < assignment.size(); ++i) {
    if (assignment(i) >= 0) {
      EXPECT_LT(assignment(i), 6)
        << "Assignment " << i << " -> " << assignment(i) << " is out of bounds";
    }
  }
